      ...
      !open the input and global output files
      OPEN(7, FILE=inpFilename)
      OPEN(8, FILE='pdb.out')
      OPEN(9, FILE='temp.arrows')
      OPEN(10, FILE='pdb.cog')
      OPEN(11, FILE='temp.cg')
      OPEN(12, FILE='temp.aa')
...
80    FORMAT(I4)
      ...
      !get the information about number of residues and which residues belong to which strand
      READ(7,81) nRb, nRe, nRS1b, nRS1e, nRS2b, nRS2e
      nRS1=nRS1e-nRS1b+1
      nRS2=nRS2e-nRS2b+1
81    FORMAT(I4,5(1X,I4))

!     write out asc nformation in .COG and stdout
      WRITE(6,81) nRb, nRe, nRS1b, nRS1e, nRS2b, nRS2e
      Write(10,72) numModel
      Write(10,72) nBB
72    FORMAT(I4)
...
73    FORMAT(I3)
      ...
      !over all centre of gravity is cumulative for each model so set to zero initially.
      OCOG(curModel,1:3)=0
 
      !Initialise running averages for this model
      aAModAv=0.0
      bAModAv=0.0
      cAModAv=0.0
      aRModAv=0.0
...
!loop through building blocks and analyse each building block on its own
      DO curBB=1,nBB,1
         ...
         !initialise position of centre of gravity
         CCOG1=0
         CCOG2=0

         !loop through all residues in current building block
         DO curR=nRb,nRe,1
 
!           WRITE(6,82) curR
82         FORMAT(I2)
 
           !read  first line of current building block
...
!           WRITE(6,100) atomNum, atomType, residueName, chain, residueNum, insert, xDum, yDum, zDum, Occupancy, TempFact, SegmentId, ElementType
           ...
           !residue number changes then bail out and back space. Scan for and Store useful information as we go.
...
!         END DO
         ...
         !output the positions of each building block to the .cog file for matching ellipsoids later on.
         write(10,84) CCOG1
         write(10,84) CCOG2
...
!       a2 and c2 axis 
        ...
        !write out centre of gravities and angle axis for each strand in separate files temp.aa and temp.cg

        write(11,94) CCOG1(1),CCOG1(2),CCOG1(3)
        write(11,94) CCOG2(1),CCOG2(2),CCOG2(3)
        write(12,94) p1(1),p1(2),p1(3)
        write(12,94) p2(1),p2(2),p2(3)
...
!      outFilename = ADJUSTL(TRIM(fileNumStr))//'/pdbC.coords'
!      OPEN(15, FILE=outFilename)
!      outFilename = ADJUSTL(TRIM(fileNumStr))//'/pdbCO.coords'
!      OPEN(16, FILE=outFilename)

      !Use the first building block orientation as the global orientation for centralising the output
!      RMatU(curModel,1:3,1)=BBX(1,1:3)
!      RMatU(curModel,1:3,2)=BBY(1,1:3)
!      RMatU(curModel,1:3,3)=BBZ(1,1:3)
...
!        CenteredPos(1)=BBCOG(curBB,1)-OCOG(curModel,1)
!        CenteredPos(2)=BBCOG(curBB,2)-OCOG(curModel,2)
!        CenteredPos(3)=BBCOG(curBB,3)-OCOG(curModel,3)
!        WRITE(15,94) CenteredPos(1),CenteredPos(2),CenteredPos(3)
!        CenteredPosRot(1)=CenteredPos(1)*BBX(1,1)+CenteredPos(2)*BBX(1,2)+CenteredPos(3)*BBX(1,3)
!        CenteredPosRot(2)=CenteredPos(1)*BBY(1,1)+CenteredPos(2)*BBY(1,2)+CenteredPos(3)*BBY(1,3)
!        CenteredPosRot(3)=CenteredPos(1)*BBZ(1,1)+CenteredPos(2)*BBZ(1,2)+CenteredPos(3)*BBZ(1,3)
!        WRITE(16,94) CenteredPosRot(1),CenteredPosRot(2),CenteredPosRot(3)
      END DO
      ...
      DO curBB=1,nBB,1
        ...
        !Compute the rotation matrix to get from the first basis to the current basis
!       RMatC(1,1)=BBX(1,1)*BBX(curBB,1)+BBX(1,2)*BBX(curBB,2)+BBX(1,3)*BBX(curBB,3)
!       RMatC(2,1)=BBY(1,1)*BBX(curBB,1)+BBY(1,2)*BBX(curBB,2)+BBY(1,3)*BBX(curBB,3)
!        RMatC(3,1)=BBZ(1,1)*BBX(curBB,1)+BBZ(1,2)*BBX(curBB,2)+BBZ(1,3)*BBX(curBB,3)
!        RMatC(1,2)=BBX(1,1)*BBY(curBB,1)+BBX(1,2)*BBY(curBB,2)+BBX(1,3)*BBY(curBB,3)
!        RMatC(2,2)=BBY(1,1)*BBY(curBB,1)+BBY(1,2)*BBY(curBB,2)+BBY(1,3)*BBY(curBB,3)
!        RMatC(3,2)=BBZ(1,1)*BBY(curBB,1)+BBZ(1,2)*BBY(curBB,2)+BBZ(1,3)*BBY(curBB,3)
!        RMatC(1,3)=BBX(1,1)*BBZ(curBB,1)+BBX(1,2)*BBZ(curBB,2)+BBX(1,3)*BBZ(curBB,3)
!        RMatC(2,3)=BBY(1,1)*BBZ(curBB,1)+BBY(1,2)*BBZ(curBB,2)+BBY(1,3)*BBZ(curBB,3)
!        RMatC(3,3)=BBZ(1,1)*BBZ(curBB,1)+BBZ(1,2)*BBZ(curBB,2)+BBZ(1,3)*BBZ(curBB,3)

        ! convert the rotation matrices to angle axis vectors.  
        CALL RMAT2AA(RMat,p1)
!        CALL RMAT2AA(RMatC,p2)
        WRITE(14,94) p1(1), p1(2), p1(3)
!        WRITE(15,94) p1(1), p1(2), p1(3)
!        WRITE(16,94) p2(1), p2(2), p2(3)
      END DO
!      CLOSE(16)
!      CLOSE(15)
      CLOSE(14)
...
!      !The c attractive parameter is guessed from the distance to the next strand - the repulsive component of the strand above.
!      DO curBB=1,nBB-1,1
!         cAModAv=cAModAv+dVertModAv-cArray(curBB+1)
!      END DO
!      cAModAv=cAModAv/(nBB-1)
...
!****** This section outputs the positions of each ellipsoid in the pdb (as opposed to the building block)
! as centred, uncentered and oriented centered coordinates


!open the tempcg file
      OPEN(9, FILE='temp.cg')

!     Loop through the models and output a finish, finishOC and finishC file for each one in the model directory
      DO curModel=1,numModel,1
 
!        Construct file name for finish, finishC and finishOC file
         WRITE(fileNumStr,136) curModel
         outFilename = ADJUSTL(TRIM(fileNumStr))//'/finish'
         OPEN(11, FILE=outFilename)
         outFilename = ADJUSTL(TRIM(fileNumStr))//'/finishC'
         OPEN(12, FILE=outFilename)
         outFilename = ADJUSTL(TRIM(fileNumStr))//'/finishOC'
         OPEN(13, FILE=outFilename)

         !Read next 2*nBB entries in .tempcg file and write as is to finish file 
         !and corrected for the overall centre of gravity for the finishC file
         DO curBB=1,nBB,1

           !read in the first ellipsoid write it out in the normal file, rotate it and write it out in the second file
           READ(9,94) CCOG1(1), CCOG1(2), CCOG1(3)
           WRITE(11,94) CCOG1(1),CCOG1(2),CCOG1(3)
           CenteredPos(1)=CCOG1(1)-OCOG(curModel,1)
           CenteredPos(2)=CCOG1(2)-OCOG(curModel,2)
           CenteredPos(3)=CCOG1(3)-OCOG(curModel,3)
           WRITE(12,94) CenteredPos(1),CenteredPos(2),CenteredPos(3)
           CenteredPosRot(1)=CenteredPos(1)*RMatU(curModel,1,1)+CenteredPos(2)*RMatU(curModel,2,1)+CenteredPos(3)*RMatU(curModel,3,1)
           CenteredPosRot(2)=CenteredPos(1)*RMatU(curModel,1,2)+CenteredPos(2)*RMatU(curModel,2,2)+CenteredPos(3)*RMatU(curModel,3,2)
           CenteredPosRot(3)=CenteredPos(1)*RMatU(curModel,1,3)+CenteredPos(2)*RMatU(curModel,2,3)+CenteredPos(3)*RMatU(curModel,3,3)
           WRITE(13,94) CenteredPosRot(1),CenteredPosRot(2),CenteredPosRot(3)
 
           !do it all again for the second ellipsoid in the unit
           READ(9,94) CCOG1(1), CCOG1(2), CCOG1(3)
           WRITE(11,94) CCOG1(1),CCOG1(2),CCOG1(3)
           CenteredPos(1)=CCOG1(1)-OCOG(curModel,1)
           CenteredPos(2)=CCOG1(2)-OCOG(curModel,2)
